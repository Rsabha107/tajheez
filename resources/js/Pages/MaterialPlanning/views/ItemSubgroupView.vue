...
const eventSubgroups = computed(() => props.event?.id ? subgroupList.value.filter(s => s.eventId === props.event.id) : subgroupList.value);
const eventGroups = computed(() => props.event?.id ? props.itemGroups.filter(g => g.eventId === props.event.id) : props.itemGroups);
...
const rows = computed(() => {
    let r = eventSubgroups.value.slice();
...
function groupLabel(code) { return eventGroups.value.find(g => g.id === code)?.label || code; }
...
function freshForm() {
    return { code: '', group: eventGroups.value[0]?.id ?? '', name: '', description: '', status: defaultStatusId() };
}
...
<template>
    <div class="mp-page">
        <div class="mp-page-head">
            <div>
                ...
                <p class="mp-page-sub">{{ eventSubgroups.length }} item subgroups · catalog items get classified against these</p>
            </div>
        ...
        <div class="filterbar">
            ...
            <div class="fb-sel">
                ...
                <select v-model="fGroup">
                    ...
                    <option v-for="g in eventGroups" :key="g.id" :value="g.id">{{ g.label }}</option>
                </select>
            </div>
        </div>
        ...
        <div class="dt-foot">Showing <b>{{ rows.length }}</b> of {{ eventSubgroups.length }} item subgroups</div>
    </div>
    ...
    <FormModal
        :show="showAdd"
        max-width="520px"
        :title="editingId ? 'Edit item subgroup' : 'New item subgroup'"
        subtitle="A named subgroup within an item group that catalog items get classified against."
        @close="closeAdd"
    >
        <template #eyebrow>
            <span class="mono">{{ event.code }}</span><span>·</span><span>Item Subgroups</span>
        </template>
        ...
        <template #footer-actions>
            <ProgressButton
                ...
                :disabled="!formValid"
                :loading="saving"
                :text="editingId ? 'Save changes' : 'Add item subgroup'"
                :loading-text="editingId ? 'Saving…' : 'Adding…'"
                @click="submitSubgroup"
            />
        </template>
    </FormModal>
