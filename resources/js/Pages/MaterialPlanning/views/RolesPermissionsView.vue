...
import RolesPermissionFormModal from '@/Components/security/RolesPermissionFormModal.vue';
...
function permStyle(i) { return permPalette[i % permPalette.length]; }
...
async function fetchData() {
    loading.value = true;
    try {
        const { data } = await axios.get(route('roles-permissions.data'), { params: { limit: 200, offset: 0, sort: 'id', order: 'desc' } });
...
async function deleteRow(row) {
    const result = await Swal.fire({ title: 'Remove role?', text: `"${row.name}"`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, remove', confirmButtonColor: '#dc2626' });
    if (!result.isConfirmed) return;
    try { await axios.delete(route('roles-permissions.destroy', row.id)); fetchData(); } catch (_) {}
}
...
<template>
    <div class="mp-page">
        <div class="mp-page-head">
            <div>
                <h1 class="mp-page-title">Roles &amp; Permissions</h1>
                <p class="mp-page-sub">Assign permissions to roles. Click a row to manage.</p>
            </div>
            <div class="mp-head-actions">
                ...
                <button class="mp-btn mp-btn-primary" @click="openCreate">+ Assign permissions</button>
            </div>
        </div>
        ...
        <div class="mp-card mp-card-flush">
            ...
            <table v-else class="mp-dt">
                ...
                <tbody>
                    <tr v-if="!filtered.length"><td colspan="3" class="mp-dt-empty">No roles found.</td></tr>
                    <tr v-for="row in filtered" :key="row.id" class="mp-dt-row" @click="openEdit(row)">
                        <td class="mp-dt-title" style="white-space:nowrap; width:180px;">{{ row.name }}</td>
                        <td>
                            <span v-if="!(row.permissions ?? []).length" class="mp-muted">No permissions</span>
                            <span v-for="(p, i) in (row.permissions ?? [])" :key="p.id ?? i"
                                class="mp-dtag" :style="{ background: permStyle(i).bg, color: permStyle(i).fg }">
                                {{ p.name ?? p }}
                            </span>
                        </td>
                        <td class="mp-dt-actions" @click.stop>
                            ...
                            <button class="mp-icon-btn mp-icon-del" title="Remove" @click="deleteRow(row)"><i class="bx bx-trash"></i></button>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="mp-dt-foot">Showing <b>{{ filtered.length }}</b> of {{ rows.length }} roles</div>

        <RolesPermissionFormModal :show="showModal" :mode="modalMode"
            :role-id="selectedRow?.id ?? null" :role-name="selectedRow?.name ?? ''"
