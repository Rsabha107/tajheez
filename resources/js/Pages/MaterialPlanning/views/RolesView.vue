...
import RoleFormModal from '@/Components/security/RoleFormModal.vue';
...
async function fetchData() {
    loading.value = true;
    try {
        const { data } = await axios.get(route('roles.data'), { params: { limit: 200, offset: 0, sort: 'id', order: 'desc' } });
...
async function deleteRow(row) {
    const result = await Swal.fire({ title: 'Delete role?', text: `"${row.name}"`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete', confirmButtonColor: '#dc2626' });
    if (!result.isConfirmed) return;
    try { await axios.delete(route('roles.destroy', row.id)); fetchData(); } catch (_) {}
}
...
<template>
    <div class="mp-page">
        <div class="mp-page-head">
            <div>
                <h1 class="mp-page-title">Roles</h1>
                <p class="mp-page-sub">All system roles. Click a row to edit.</p>
            </div>
            <div class="mp-head-actions">
                ...
                <button class="mp-btn mp-btn-primary" @click="openCreate">+ Add role</button>
            </div>
        </div>
        ...
        <div class="mp-card mp-card-flush">
            ...
            <table v-else class="mp-dt">
                ...
                <tbody>
                    <tr v-if="!filtered.length"><td colspan="2" class="mp-dt-empty">No roles found.</td></tr>
        ...
        <RoleFormModal :show="showModal" :mode="modalMode"
