...
import VenueFormModal from '@/Components/ems/VenueFormModal.vue';
...
async function fetchData() {
    loading.value = true;
    try {
        const { data } = await axios.get(route('venues.data'), { params: { limit: 200, offset: 0, sort: 'id', order: 'desc' } });
...
async function deleteRow(row) {
    const result = await Swal.fire({ title: 'Delete venue?', text: `"${row.title}"`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete', confirmButtonColor: '#dc2626' });
    if (!result.isConfirmed) return;
    try { await axios.delete(route('venues.destroy', row.id)); fetchData(); } catch (_) {}
}
...
<template>
    <div class="mp-page">
        <div class="mp-page-head">
            <div>
                <h1 class="mp-page-title">Venues</h1>
                <p class="mp-page-sub">All venues in the system. Click a row to edit.</p>
            </div>
            <div class="mp-head-actions">
                ...
                <button class="mp-btn mp-btn-primary" @click="openCreate">+ Add venue</button>
            </div>
        </div>
        ...
        <div class="mp-filterbar">
            <div class="mp-fb-search">
                ...
                <input v-model="search" placeholder="Search venues…" />
            </div>
            <div class="mp-fb-sel">
                <label>Status</label>
        ...
        <div class="mp-card mp-card-flush">
            ...
            <table v-else class="mp-dt">
                ...
                <tbody>
                    <tr v-if="!filtered.length"><td colspan="4" class="mp-dt-empty">No venues found.</td></tr>
        ...
        <div class="mp-dt-foot">Showing <b>{{ filtered.length }}</b> of {{ rows.length }} venues</div>

        <VenueFormModal :show="showModal" :mode="modalMode" :venue="selectedRow"
