...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement('UPDATE mp_request_lines l JOIN mp_catalog_items c ON l.sku = c.sku SET l.catalog_item_id = c.id');

        Schema::table('mp_request_lines', function (Blueprint $table) {
            $table->dropColumn('sku');
        });

        DB::statement('ALTER TABLE mp_request_lines MODIFY catalog_item_id BIGINT UNSIGNED NOT NULL');
    ...
    public function down(): void
    {
        Schema::table('mp_request_lines', function (Blueprint $table) {
            ...
            $table->string('sku', 20)->nullable()->after('request_id');
        });

        DB::statement('UPDATE mp_request_lines l JOIN mp_catalog_items c ON l.catalog_item_id = c.id SET l.sku = c.sku');

        Schema::table('mp_request_lines', function (Blueprint $table) {
            $table->dropColumn('catalog_item_id');
        });

        DB::statement('ALTER TABLE mp_request_lines MODIFY sku VARCHAR(20) NOT NULL');
